...
def bytes_to_service_info(payload: bytes) -> BluetoothServiceInfo:
    return BluetoothServiceInfo(
        ...
        rssi=-60,
        manufacturer_data={},
        service_data={"0000fe95-0000-1000-8000-00805f9b34fb": payload[17:]},
        service_uuids=["0000fe95-0000-1000-8000-00805f9b34fb"],
        source="",
    )


def hex_to_service_info(payload: str) -> BluetoothServiceInfo:
    return bytes_to_service_info(bytes.fromhex(payload))
...
def test_Xiaomi_LYWSDCGQ(caplog):
    ...
    advertisement = hex_to_service_info(data_string)
...
def test_Xiaomi_CGG1():
    ...
    assert device.supported(advertisement)
    assert device.update(advertisement)

    assert device.descriptions == {
...
def test_Xiaomi_CGDK2():
    ...
    device = XiaomiBluetoothDeviceData()
    assert device.supported(hex_to_service_info(data_string))
...
def test_Xiaomi_XMMF01JQD():
    ...
    data_string = (
        "043e240201000154d3e63053e218020106141695fe5030e1048e54d3e63053e2011003010000b8"
    )
    data = bytes(bytearray.fromhex(data_string))
...
def test_Xiaomi_M1S_T500():
    ...
    data_string = (
        "043e2402010001115b174371e618020106141695fe7130890437115b174371e6091000020003dc"
    )
    data = bytes(bytearray.fromhex(data_string))
